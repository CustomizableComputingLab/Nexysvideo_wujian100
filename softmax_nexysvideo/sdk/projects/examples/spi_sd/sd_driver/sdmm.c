...
PA18		CARD DETECT
PA19		CS
PA20		HIGHT RESET , LOW PORWER THE CARD
SPI2		                
*/
/*-----------------------------------------------------------------------*/
// #define 		sd_clk_freq		1000000
#define 		sd_clk_freq		300000
#define			CS_H()			csi_gpio_pin_write(sd_cs_pin_handle_t,1)
#define			CS_L()			csi_gpio_pin_write(sd_cs_pin_handle_t,0)

spi_handle_t sd_spi_handle_t;
...
{	
	int32_t ret;		
																					   //usi2									usi1
    sd_cd_pin_handle_t = csi_gpio_pin_initialize(PA18, gpio_interrupt_handler);        //sd_cd_pin_handle_t PA18 				PA18	
    ...
    sd_cs_pin_handle_t = csi_gpio_pin_initialize(PA27, gpio_interrupt_handler);        //sd_cs_pin_handle_t PA19 				PA27
    ...
    sd_rst_pin_handle_t = csi_gpio_pin_initialize(PA28, gpio_interrupt_handler);        //sd_rst_pin_handle_t PA20 				PA28
    ...
    sd_spi_handle_t = csi_spi_initialize(1, spi_event_cb_fun);             				//sd usi 2   							USI1
    if (sd_spi_handle_t == NULL) {	
        printf(" csi_spi_initialize failed\n");
        return -1;
    }
    ret = csi_spi_config(sd_spi_handle_t, sd_clk_freq, SPI_MODE_MASTER,
...
{
	csi_spi_send(sd_spi_handle_t,buff,1);
}
...
)
{
	...
	uint8_t *rx_index = buff;
	uint32_t i = 0;
	//DI_H();	/* Send 0xFF */															//how to keep mosi high
	// csi_spi_receive(sd_spi_handle_t,buff,bc);
	for(i=0;i<bc;i++)
	{
		csi_spi_transfer(sd_spi_handle_t,&tx_data,rx_index,1,1);
		rx_index++;
	}
}
...
)
{
	...
	res = SD_SPI_Init();
	if(res != 0)
		printf("SD_SPI_Init error\r\n");
	else
		printf("SD_SPI_Init successfully\r\n");
	...
	for(i=0;i<100;i++)
		tx_buff[i] = 0xff;
	csi_spi_transfer(sd_spi_handle_t,tx_buff,rx_buff,100,100);
